#!/usr/bin/env python3
"""
Quick CLI search utility for MyMemex.
...
def search(query: str, mode: str = "keyword", limit: int = 10, port: int = 8000):
    ...
    try:
        ...
        print(f"[FAIL] Could not connect to MyMemex REST API: {e}")
        print(f"  Make sure the server is running: mymemex serve")
        sys.exit(1)


def main():
    parser = argparse.ArgumentParser(description="Quick search MyMemex documents")
