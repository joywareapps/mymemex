...
class IngestService:
    ...
    async def upload(self, content: bytes, filename: str) -> dict:
        ...
        if not self.config.watch.directories:
            raise ServiceError("No watch directories configured")

        upload_dir = Path(self.config.watch.directories[0]).expanduser() / "_uploads"
