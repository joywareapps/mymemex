...
def _get_ctx(ctx: Context) -> MyMemexContext:
...
def register(mcp: FastMCP) -> None:
    ...
    async def list_documents(
        ...
        Args:
            ...
            status: Filter by status (pending, processing, processed, failed, waiting_llm, waiting_ocr).
    ...
    async def upload_document(
        ...
        try:
            async with get_session() as session:
                ...
                if file_path:
                    allowed = lctx.config.mcp.security.allowed_parent_paths or []
