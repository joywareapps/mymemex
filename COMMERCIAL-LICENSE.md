# Commercial Licensing

MyMemex is open source under AGPL v3, which is great for personal use, 
self-hosting, and open-source contributions. However, AGPL has requirements 
that may not suit all commercial use cases.

## When You Need a Commercial License

| Use Case | AGPL v3 OK? | Commercial License |
|----------|-------------|-------------------|
| Personal/home use | ✅ Yes | Not needed |
| Self-hosted for your business | ✅ Yes | Not needed |
| Contributing to open source | ✅ Yes | Not needed |
| SaaS product using MyMemex | ❌ No | **Required** |
| Proprietary product embedding MyMemex | ❌ No | **Required** |
| Distributing modified closed-source version | ❌ No | **Required** |
| Want guaranteed support/SLA | ❌ No | **Recommended** |

## What Commercial License Includes

- **No AGPL obligations** — Keep your modifications proprietary
- **SaaS resale rights** — Use in your hosted product
- **Priority email support** — 48-hour response time
- **Custom development** — Feature requests prioritized
- **SLA options** — Uptime guarantees for enterprise

## Pricing

| License Type | Price | Use Case |
|--------------|-------|----------|
| Startup | $499/year | < $100k revenue, <5 employees |
| Business | $1,999/year | < $1M revenue, <20 employees |
| Enterprise | Custom | Larger organizations, custom terms |

All licenses include:
- Unlimited users/documents
- All current and future features
- Email support

## Contact

For commercial licensing inquiries:

- **Email:** support@joywareapps.com
- **Website:** https://joywareapps.com
- **Response time:** Within 2 business days

---

*Pricing subject to change. Quotes valid for 30 days.*
